CREATE OR REPLACE PROCEDURE STG.BUILD_AND_EXECUTE_MERGE_FOR_DIMENSION_DEST_REPAIR (STG_SCHEMA VARCHAR, 
																				  STG_TABLE VARCHAR, 
																				  EDW_SCHEMA VARCHAR, 
																				  EDW_TABLE VARCHAR, 
...
sql = `DELETE FROM dst_table 
		WHERE COLUMN_NAME IN (SELECT ROW_IS_CURRENT FROM STG.ADDED_DIM_COLUMN_NAMES WHERE ADDED_DIM_COLUMN_NAMES_TAG = ADDED_DIM_COLUMN_NAMES_TAG)
		   OR COLUMN_NAME IN (SELECT ROW_EFFECTIVE_DATE FROM STG.ADDED_DIM_COLUMN_NAMES WHERE ADDED_DIM_COLUMN_NAMES_TAG = ADDED_DIM_COLUMN_NAMES_TAG)
		   OR COLUMN_NAME IN (SELECT ROW_EXPIRATION_DATE FROM STG.ADDED_DIM_COLUMN_NAMES WHERE ADDED_DIM_COLUMN_NAMES_TAG = ADDED_DIM_COLUMN_NAMES_TAG)
		   OR COLUMN_NAME IN (SELECT ROW_INSERT_DATE FROM STG.ADDED_DIM_COLUMN_NAMES WHERE ADDED_DIM_COLUMN_NAMES_TAG = ADDED_DIM_COLUMN_NAMES_TAG)
		   OR COLUMN_NAME IN (SELECT ROW_UPDATE_DATE FROM STG.ADDED_DIM_COLUMN_NAMES WHERE ADDED_DIM_COLUMN_NAMES_TAG = ADDED_DIM_COLUMN_NAMES_TAG)
