...
*/
CREATE OR REPLACE PROCEDURE Z_DEBUG.SNOWFLAKE_DATA_COMPARE(SRC_DATABASE VARCHAR, SRC_SCHEMA VARCHAR, SRC_TABLE VARCHAR,
                                        TGT_DATABASE VARCHAR, TGT_SCHEMA VARCHAR, TGT_TABLE VARCHAR,
                                        NATURAL_KEY_COLUMNS VARCHAR, IGNORE_COLUMNS VARCHAR,
                                        SOURCE_WHERE_CLAUSE VARCHAR, DESTINATION_WHERE_CLAUSE VARCHAR,
                                        OUTPUT_DATABASE VARCHAR, OUTPUT_SCHEMA VARCHAR, OUTPUT_TABLE VARCHAR,
                                        CREATE_OUTPUT_TABLE VARCHAR, DEBUG_MODE VARCHAR)
    returns VARCHAR
    language JAVASCRIPT
...
sql = `WITH Difference_Check AS ( ` + Difference_SELECT_final + ` )
		, SRC_ONLY AS (SELECT 'Only In Source' AS Type_Code
		                  ...
		                  FROM (SELECT * FROM ` + stg_table_full + ` ` + SOURCE_WHERE.replace(/''/g, "'") + `) src
		                  LEFT JOIN (SELECT * FROM ` + tgt_table_full + ` ` + TARGET_WHERE.replace(/''/g, "'") + `) tgt
		                    ON ` + StagingNatKeyJoin + `
		...
		, TGT_ONLY AS (SELECT 'Only In Destination' AS Type_Code
								...
								, NULL AS Source_Value
								, NULL AS Target_Value
		                  FROM (SELECT * FROM ` + tgt_table_full + ` ` + TARGET_WHERE.replace(/''/g, "'") + `) tgt
		                  LEFT JOIN (SELECT * FROM ` + stg_table_full + ` ` + SOURCE_WHERE.replace(/''/g, "'") + `) src
		                    ON ` + StagingNatKeyJoin + `
